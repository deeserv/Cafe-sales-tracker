...
PROJECT_STORE_MAPPING = {
    ...
    "百度项目": ["度咖啡（百度鹏寰店）", "度小满店", "度咖啡（百度科技园店）", "度咖啡（百度大厦店）", "度咖啡（百度奎科店）", "度咖啡（百度上研店）"],
...
def logic_clean_data(df):
    ...
    if missing:
        st.error(f"⚠️ 报表格式不匹配，缺少关键列：{', '.join(missing)}")
        st.stop()
    ...
    with st.sidebar.expander("📥 数据上传 (本地模式)", expanded=True):
        ...
        if files:
            ...
            for f in files:
                ...
                # 🌟 智能列名映射转换 (涵盖更多可能性)
                mapping = {
                    '商品实收': '销售金额', '实收金额': '销售金额', '销售额': '销售金额',
                    '商品销量': '销售数量', '销售数': '销售数量', '数量': '销售数量',
                    '日期': '统计周期', '时间': '统计周期',
                    '商品分类': '商品类别', '商品类别': '商品类别', '分类': '商品类别'
                }
                # 执行转换
                df_u = df_u.rename(columns={k: v for k, v in mapping.items() if k in df_u.columns})
                all_dfs.append(df_u)
