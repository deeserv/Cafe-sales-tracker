...
# =============================================================================
# 1. 业务逻辑配置：映射关系表 (项目归属与品类层级)
# =============================================================================
PROJECT_STORE_MAPPING = {
    "光大项目": ["光大咖啡上地店", "光大咖啡上海分行店", "光大咖啡总行店"],
    "百度项目": ["度咖啡（百度鹏寰店）", "度小满店", "度咖啡（百度科技园店）", "度咖啡（百度大厦店）", "度咖啡（百度大厦店）", "度咖啡（百度奎科店）", "度咖啡（百度上研店）"],
...
def logic_clean_data(df):
    ...
    # 🌟 核心修复 1：在任何操作前彻底重置索引，确保索引唯一且连续
    df = df.copy().reset_index(drop=True)
    
    # 清洗列名
    df.columns = [str(c).strip().replace('\n', '').replace('\r', '') for c in df.columns]
    
    # 🌟 核心修复 2：防止列名重复导致的赋值错误
    rename_map = {}
    seen_targets = set()
    standard_map = {
        '商品类别': ['商品类别', '分类', '品类'],
    ...
    for col in df.columns:
        for target, keywords in standard_map.items():
            if target not in seen_targets and any(k in col for k in keywords):
    ...
    # 使用 .values 确保赋值时不依赖索引对齐，彻底解决 reindex 报错
    df['所属项目'] = df['门店名称'].apply(lambda x: s2p.get(str(x).strip(), '其他项目')).values
    
    # 品类层级映射 (先去重映射表防止 merge 导致数据膨胀)
    ...
    with st.sidebar.expander("📥 数据上传 (本地模式)", expanded=True):
        ...
        if files:
            ...
            if all_dfs:
                ...
                st.session_state.sales_data = combined.drop_duplicates().reset_index(drop=True)
                st.success("数据加载成功")

    if st.session_state.sales_data.empty:
        st.info("💡 请先在左侧上传报表以开启看板分析。")
        return
...
if __name__ == "__main__":
    ...
    if menu == "📊 经营看板":
        ...
        st.title("⚙️ 成本配方中心")
        st.info("框架已就绪。在确保看板读取正常后，我们开始完善这里的配方逻辑。")
